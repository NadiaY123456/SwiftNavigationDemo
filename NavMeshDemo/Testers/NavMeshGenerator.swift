...
let customConfig = NavMeshBuilder.Config(
    width: nil, // computed from bmin/bmax & cellSize
    height: nil, // computed from bmin/bmax & cellSize
    tileSize: 32, // size of each tile (in voxels)
    borderSize: nil, // defaults to walkableRadius + 3
    cellSize: 0.03, // xz-plane cell size (world units)
    cellHeight: 0.2, // y-axis cell size (world units)
    bmin: nil, // minimum AABB corner
    bmax: nil, // maximum AABB corner
    partitionStyle: .watershed, // use Recast’s watershed partitioning
    walkableSlopeAngle: 45, // maximum walkable slope (degrees)
    walkableHeight: 2, // agent height (in voxels)
    walkableClimb: 1, // max climbable step (in voxels)
    walkableRadius: 1, // agent radius (in voxels)
    maxEdgeLen: 15, // max allowed contour edge length (voxels)
    maxSimplificationError: 1.3, // contour simplification tolerance (voxels)
    minRegionArea: 64, // filter out regions smaller than this (voxels²)
    mergeRegionArea: 400, // merge regions smaller than this (voxels²)
    maxVertsPerPoly: 6, // max vertices per polygon
    detailSampleDist: 6, // sampling distance for detail mesh (world units)
    detailSampleMaxError: 1, // detail mesh height error tolerance (world units)
    filterLowHangingObstables: false, // don’t allow low‐hanging obstacles to be walkable
    filterLedgeSpans: false, // don’t treat ledges as walkable
    filterWalkableLowHeightSpans: false // don’t filter out low‐height spans
)
...
public enum NavMeshGenerator {
    ...
    public static func makeNavMesh(
        ...
        config: NavMeshBuilder.Config = NavMeshBuilder.Config(),
        agentHeight: Float,
        agentRadius: Float,
        agentMaxClimb: Float
        ...
        case .model(let name, let rotation):
            ...
            guard let modelComponent = entity.model else {
                throw NSError(
        ...
        var areaDefinitions = [NavMeshBuilder.AreaDefinition]()
        for descriptor in splats {
            for channelInfo in descriptor.channels {
                ...
                let areaDef = NavMeshBuilder.AreaDefinition(
